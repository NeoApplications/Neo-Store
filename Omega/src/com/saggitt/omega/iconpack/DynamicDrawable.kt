...
class DynamicDrawable {
    companion object {
        ...
        fun drawIcon(
            ...
            metadata.load(context, iconDpi)
            return when (metadata.type) {
                Type.CLOCK -> drawableFactory.customClockDrawer.drawIcon(
                    icon,
                    metadata.clockMetadata!!.drawable,
        ...
        fun getDrawable(
            ...
            return try {
                ...
                if (id != 0) res.getDrawableForDensity(id, density, null) else null
            ...
            if (!loaded) {
                ...
                while (parser != null && parser.next() != XmlPullParser.END_DOCUMENT) {
                    if (parser.eventType == XmlPullParser.START_TAG && parser.name == "item") {
                        val item = Item(parser["type"]!!, parser["key"]!!)
                ...
                if (type == Type.CLOCK) loadClock(context, iconDpi)
                loaded = true
            }
        }
            ...
            for (relevantItem in relevantItems) {
                ...
                if (item != null) {
                    if (item.drawables.size > 1) {
                        Log.e(
                            TAG, "Unexpected number of drawables ${item.drawables.size}! " +
    ...
    internal class TextAttributes(
        val alignX: String?, val alignY: String?, val offsetX: String?,
    ...
    internal enum class Type { CALENDAR, CLOCK, WEATHER }
}